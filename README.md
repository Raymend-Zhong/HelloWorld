...
- 家长创建和编辑按星期安排的打卡目标，查看列表与详情；
- 每个目标独立保存门槛、奖励、计划天数、内置养成活动及任务规则快照；
- 孩子端按业务日期查看今日任务和各自目标，同一任务合并显示；
- 停用任务保留在原目标中，不能加入新目标；
- 模式 2 升级到模式 3，保留已有任务与家长凭证。

后续任务将加入打卡、清算、目标结束和虚拟养成反馈。详细验收见 [TASK-5 实施与验收](docs/implementation/TASK-5-实施与验收.md)。
